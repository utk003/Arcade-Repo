...
public abstract class Ghost extends BoardPlayer {
    ...
    protected void loadSprites(String color) {
        ...
        for (String dir : dirs)
            for (int i = 1; i <= 2; i++) {
                sprites[ind] = new ImageIcon("assets/pacman/ghosts/" + color + dir + i + ".png").getImage();
                ind++;
            }
        for (String c : scareCol)
            for (int i = 1; i <= 2; i++) {
                sprites[ind] = new ImageIcon("assets/pacman/ghosts/scared" + c + i + ".png").getImage();
                ind++;
            }
        for (String dir : dirs) {
            sprites[ind] = new ImageIcon("assets/pacman/ghosts/eyes" + dir + ".png").getImage();
            ind++;
        }
    }
