...
public class BlockDisplay extends JComponent implements KeyListener, MouseListener
{
    ...
    public BlockDisplay(MyBoundedGrid<Block> board)
    {
        ...
        isMusicOn = false;
        URL url = getClass().getClassLoader().getResource("graphics/kahoot.wav");
    ...
    {
        Image image = new ImageIcon("graphics/music.png").getImage();
        g.drawImage(image, x, y, BLOCKSIZE * 2, BLOCKSIZE * 2, null);
    }
    
    private void drawPause(Graphics g, int x, int y)
    {
        Image image = new ImageIcon("graphics/pause.png").getImage();
        g.drawImage(image, x, y, BLOCKSIZE * 2, BLOCKSIZE * 2, null);
    }
    
    private void drawPlay(Graphics g, int x, int y)
    {
        Image image = new ImageIcon("graphics/play.png").getImage();
        g.drawImage(image, x, y, BLOCKSIZE * 2, BLOCKSIZE * 2, null);
        if (!listener.hasLost())
        {
    ...
    {
        Image image = new ImageIcon("graphics/drop4.png").getImage();
        g.drawImage(image, x, y, BLOCKSIZE * 2, BLOCKSIZE * 2, null);
    }
    
    private void drawNewGame(Graphics g, int x, int y)
    {
        Image image = new ImageIcon("graphics/newGame.png").getImage();
        g.drawImage(image, x, y, BLOCKSIZE * 2, BLOCKSIZE * 2, null);
    }
